package LeetCode.LC900_;

import java.util.ArrayList;
import java.util.Comparator;
import java.util.PriorityQueue;
import java.util.Queue;

/**
 * K Closest Points to Origin
 *
 * We have a list of points on the plane.  Find the K closest points to the origin (0, 0).
 *
 * (Here, the distance between two points on a plane is the Euclidean distance.)
 *
 * You may return the answer in any order.  The answer is guaranteed to be unique (except for the order that it is in.)
 *
 *
 *
 * Example 1:
 *
 * Input: points = [[1,3],[-2,2]], K = 1
 * Output: [[-2,2]]
 * Explanation:
 * The distance between (1, 3) and the origin is sqrt(10).
 * The distance between (-2, 2) and the origin is sqrt(8).
 * Since sqrt(8) < sqrt(10), (-2, 2) is closer to the origin.
 * We only want the closest K = 1 points from the origin, so the answer is just [[-2,2]].
 * Example 2:
 *
 * Input: points = [[3,3],[5,-1],[-2,4]], K = 2
 * Output: [[3,3],[-2,4]]
 * (The answer [[-2,4],[3,3]] would also be accepted.)
 *
 *
 * Note:
 *
 * 1 <= K <= points.length <= 10000
 * -10000 < points[i][0] < 10000
 * -10000 < points[i][1] < 10000
 */
public class LC0973 {
    public int[][] kClosestHeap(int[][] points, int K) {
        if (points == null || points.length == 0 || K < 0 || K >= points.length) {
            return points;
        }

        Comparator<int[]> comp = new Comparator<int[]>() {
            @Override
            public int compare(int[] o1, int[] o2) {
                return (o2[0] * o2[0] + o2[1] * o2[1]) - (o1[0] * o1[0] + o1[1] * o1[1]);
            }
        };
        Queue<int[]> heap = new PriorityQueue<>(comp);
        for (int i = 0; i < K; i++) {
            heap.offer(points[i]);
        }
        for (int i = K; i < points.length; i++) {
            if (comp.compare(heap.peek(), points[i]) < 0) {
                heap.poll();
                heap.offer(points[i]);
            }
        }

        int[][] res = new int[K][];
        int j = 0;
        for (int[] point : heap) {
            res[j++] = point;
        }
        return res;
    }


    // quick select method
    public int[][] kClosest(int[][] points, int K) {
        if (points == null || K < 0 || K >= points.length) {
            return points;
        }

        quickSelect(points, 0, points.length - 1, K);
        int[][] res = new int[K][];
        for (int i = 0; i < K; i++) {
            res[i] = points[i];
        }
        return res;
    }

    private void quickSelect(int[][] points, int start, int end, int k) {
        if (start >= k || end < k) {
            return;
        }

        int left = start;
        int right = end - 1;
        while (left <= right) {
            if (compare(points, left, end) > 0 && compare(points, right, end) <= 0) {
                swap(points, left++, right--);
                continue;
            }
            if (compare(points, left, end) <= 0) {
                left++;
            }
            if (compare(points, right, end) > 0) {
                right--;
            }
        }
        swap(points, left, end);

        quickSelect(points, start, left - 1, k);
        quickSelect(points, left + 1, end, k);
    }

    private int compare(int[][] points, int i, int j) {
        int dist1 = points[i][0] * points[i][0] + points[i][1] * points[i][1];
        int dist2 = points[j][0] * points[j][0] + points[j][1] * points[j][1];
        return dist1 - dist2;
    }

    private void swap(int[][] array, int i, int j) {
        int[] temp = array[i];
        array[i] = array[j];
        array[j] = temp;
    }
}
